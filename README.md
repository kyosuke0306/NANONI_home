...
4. **商品情報の差し替え** — 商品は `NANONI -white-` の 1 点です。商品名以外（アルコール度数 `Alc. 0.0%`、IBU、容量、価格 `¥0,000`、味わいの説明、スタイル表記 `White Ale`）は未確定のため、ひと目で仮とわかる値を入れてあります。`products.html` と `index.html` の 2 か所を実際の値に差し替えてください。
