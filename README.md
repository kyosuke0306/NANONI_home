...
4. **商品情報の差し替え** — 商品は `NANONI -white-` の 1 点です。商品名・写真・アルコール度数（5%）・`White Fig` の表記は缶のラベルどおりです。**容量（`000ml`）と価格（`¥0,000`）、味わいの説明は未確定**のため、ひと目で仮とわかる値を入れてあります。`products.html` と `index.html` の 2 か所を差し替えてください。
